...
impl Session {
    ...
    fn try_group_by_interval(&self, interval: Milliseconds) -> Option<Vec<GroupRecord>> {
...
impl Session {
    ...
    /// Tries to group by a time interval,
    /// draws a png and returns its filename if succeeds,
    /// or returns `None` if fails.
    pub fn save_group_by_interval(
        &self,
        image_dir: &str,
        interval: Milliseconds,
    ) -> Option<String> {
        match self.try_group_by_interval(interval) {
            Some(groups) => {
                let image_name = format!("session_{}_grouping_{}.png", self.id, interval);
                ...
                match self.draw_group_by_interval(groups, interval, count_max, &image_path) {
                    Ok(()) => Some(image_name),
                    Err(_) => None,
                }
            }
    ...
    pub fn draw_group_by_interval(
        ...
        let mut chart = ChartBuilder::on(&root)
            .caption(&caption, ("Consolas", 40).into_font())
            .margin(40)
        ...
        chart
            .configure_mesh()
            .label_style(("Consolas", 28).into_font())
            .axis_desc_style(("Consolas", 32).into_font())
            .x_desc("Range / sec")
            .y_desc("Count")
            .draw()?;
